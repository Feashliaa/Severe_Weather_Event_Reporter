...
An AI-powered post-event severe weather report generator. Input a location, date, and time window - the tool fetches NWS warnings, Local Storm Reports (IEM), NCEI Storm Events survey data, pre-event upper air soundings, and archived NEXRAD Level II radar, then produces a polished HTML report with an animated radar loop, interactive event map, Skew-T sounding analysis, volume scan analysis table, and a structured LLM narrative grounded entirely in real meteorological data.
...
- **Impact summary card** - EF rating, fatalities, injuries, warning lead time, path length, max reflectivity, echo tops, warning count, LSR count, and NCEI property damage at a glance
...
- **Pre-event sounding** - Skew-T log-P diagram with parcel path, shaded CAPE/CIN, and dedicated wind barb panel, plus computed indices (CAPE, CIN, 0-6km bulk shear, surface conditions) from the nearest upper air station
- **AI narrative** - structured overview, environmental context, storm evolution, warnings issued, impacts and conclusion. Written from extracted numeric radar features and structured warning/LSR/NCEI/sounding data, not hallucinated from images
...
- **Interactive event map** - Leaflet map with warning polygons, LSR points, and NCEI tornado tracks color-coded by EF rating with approximate width corridors
...
Reports take about 3-5 minutes to generate (radar rendering is the bottleneck). Progress is shown live on the status page with a time estimate.
...
│   │   └── sounding.py             # IEM RAOB + Wyoming fallback, MetPy indices, Skew-T
│   │
│   ├── llm/
...
| Hackleburg, AL tornado | April 27, 2011 | Part of 2011 Super Outbreak, Wyoming sounding fallback |
...
**Done:**
- End-to-end pipeline (warnings -> LSRs -> NCEI -> sounding -> radar -> LLM -> report)
...
- NCEI Storm Events integration (multi-county, episode IDs)
...
- Impact summary card
- Interactive Leaflet event map with warning polygons + LSR points + tornado tracks with width corridors
- Concurrent pipeline prevention

**Planned:**
- Hodograph panel + storm relative helicity (SRH)
...
- NCEI tornado records are split by county. Path lengths shown reflect the longest single county segment; the map displays all segments. Tracks are drawn as straight lines between recorded start/end coordinates - actual paths may curve.
